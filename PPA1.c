...
// These are the six required program loop functions to build our "McMaster Terminal Program Engine"
...
//  2. Allocated memories for dynamic variables (we will talk about this after the midterm)
...
//                                          Therefore, Vending Machine operates in Synchronous Input mode - one action per command (conceptually)
...
//                       Real-Life Example:  YouTube Music will continue playing music even if you left it unattended.
//                                           It will never stop playing music every 10 seconds to wait for you to tell it to continue playing. (Won't that be annoying?)
//                                           Therefore, YouTube Music operates in Asynchronous Input mode - execution continues without commands; commands only change program behaviour.
...
void GetInput(void)
{
    ...
    //   1. Check whether there is any unprocessed input character - read the PPA1 manual and see which MacUILib function you need to use.
    //   2. If there is an input character waiting to be processed, get the character and store it as the "command"
    //      - again, read the PPA1 manual to find out which MacUILib function you need to use.
...
// MAIN LOGIC ROUTINE - "Think/Reason"
// ===============================
...
void RunLogic(void)
{
    ...
    //           1.1.a  If input command is 'ESC' (or your choice of ASCII character to signal program shutdown)
...
// If we apply the animation technique discussed in the PPA1 manual, we can then create a smooth marquee display animation on the terminal.
//
...
//
//  If we repeat the actions above at a suitable rate, the contents on the screen will look like they are moving smoothly on the screen,
...
void LoopDelay(void)
{
    // [TODO]: For now, just call the MacUILib_Delay routine here, and introduce a sufficient delay constant so that the marquee display runs at a comfortable speed.
